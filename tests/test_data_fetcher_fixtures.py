...
def test_pagination_uses_fixtures_and_deduplicates(
    ...
    page_1 = load_fixture(
        "gamma_markets_page_1.json"
    )

    page_2 = load_fixture(
        "gamma_markets_page_2.json"
    )
    ...
    markets = (
        fetcher.fetch_all_markets_paginated(
            max_pages=10
        )
    )
...
def test_failed_page_does_not_silently_skip_to_later_offset(
    ...
    def fake_api_get(
        ...
        raise AssertionError(
            "pagination must stop instead "
            "of skipping a failed page"
        )
