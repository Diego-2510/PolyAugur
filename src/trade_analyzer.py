...
class TradeAnalyzer:
    ...
    def _compute_metrics(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        top_wallet, top_wallet_vol = (
            wallet_volumes.most_common(1)[0] if wallet_volumes else ("none", 0)
        )
...
def main():
    ...
    print(
        f"   Whale trades:      {metrics['whale_count']} (${metrics['whale_volume_pct']:.0%} of vol)"
    )
