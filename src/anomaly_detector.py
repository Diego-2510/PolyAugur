...
class AnomalyDetector:
    ...
    def __init__(self):
        ...
        logger.info(
            f"🔍 AnomalyDetector initialized | Phase 15 Blacklist | threshold: {self.confidence_threshold}"
        )
    ...
    def calculate_topic_sensitivity(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        critical_topics = [
            ...
            "ceo step",
            "ceo replace",
            "chief executive resign",
            "chief executive fired",
            "board of directors",
            "shareholder vote",
            "ipo price",
            "ipo date",
    ...
    def detect_anomaly(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            base_score = volume_result["score"] + price_result["score"] + holder_result["score"]
            ...
            if anomaly_detected:
                ...
                logger.debug(
                    f"🚫 Excluded: {snapshot.get('question', '')[:55]} | {topic_result['reasons']}"
                )
            else:
                logger.debug(
                    f"✓ Clean: {snapshot.get('question', '')[:55]} | Score: {final_score:.3f}"
                )
    ...
    def batch_detect(self, snapshots: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        critical_count = sum(
            ...
            if any(
                ...
                for reason in r.get("breakdown", {}).get("topic_sensitivity", {}).get("reasons", [])
            )
        )
        elevated_count = sum(
            1
